...
const ExcelImporter: React.FC<ExcelImporterProps> = ({ onImportComplete }) => {
  ...
  const detectSheetType = (data: any[]): 'faturamento' | 'estoque' | 'colaboradores' | 'unknown' => {
    ...
    const estoqueIndicators = [
      ...
      'estoque final', 'dia estocad', 'sugrida'
    ];
  ...
  const parseColaboradoresData = (data: any[]): ColaboradorData[] => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // Detectar se é uma linha de usuário
      if (rowString.includes('usuário:') || rowString.includes('colaborador:')) {
        const userMatch = rowString.match(/usuário:\s*([^-]+)-(\d+)/i);
        if (userMatch) {
          currentUser = userMatch[1].trim();
          currentUserId = userMatch[2].trim();
          console.log(`👤 Encontrado usuário: ${currentUser} (ID: ${currentUserId})`);
          continue;
        }
      }
  ...
  const parseEstoqueData = (data: any[]): EstoqueData[] => {
    ...
    if (headerRowIndex === -1) {
      ...
      Object.keys(headerRow).forEach(key => {
        ...
        if (headerValue.includes('un. neg.') || headerValue.includes('unidade') || 
            headerValue.includes('cód.') || headerValue.includes('codigo') ||
            headerValue.includes('código') || headerValue.includes('cód')) {
  ...
  const importEstoqueData = async (estoqueData: EstoqueData[]) => {
    ...
    // Criar mapa adicional para buscar por nome (fallback)
    const unidadesPorNome = new Map(unidades?.map((u: any) => [u.nome, u.id]) || []);

    console.log('🏢 Mapeamento de unidades:', {
      porCodigo: Array.from(unidadesMap.entries()),
      porNome: Array.from(unidadesPorNome.entries())
    });
    ...
    for (const item of estoqueData) {
      ...
      if (!unidadeId) {
        unidadeId = unidadesPorNome.get(unidadeCode);
        if (unidadeId) {
          console.log(`🔄 Unidade encontrada por nome: ${unidadeCode} -> ID: ${unidadeId}`);
        }
      }
      ...
      if (unidadeId) {
        ...
        console.log(`⚠️ Unidade não encontrada (código/nome): ${unidadeCode}`);
      }
      ...
      reader.onload = async (e) => {
        ...
        workbook.SheetNames.forEach((sheetName, sheetIndex) => {
          ...
          if (sheetType === 'estoque') {
            ...
            const colaboradoresData = parseColaboradoresData(jsonData);
